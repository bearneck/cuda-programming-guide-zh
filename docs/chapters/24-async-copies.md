...
| 方向 | 异步复制 (LDGSTS, CC 8.0+) |  |  |
| --- | --- | --- | --- |
| 源 | 目标 | 完成机制 | API |
| 全局内存 | 全局内存 |  |  |
| 共享内存::cta | 全局内存 |  |  |
| 全局内存 | 共享内存::cta | 共享内存屏障,
...
__global__ void stencil_kernel(const float *left, const float *center, const float *right)
{
    ...
    // 左侧光晕（8个元素）- 中心（32个元素）- 右侧光晕（8个元素）。
    __shared__ float buffer[8 + 32 + 8];

    // 在所有线程间协作发起复制操作。
    cg::memcpy_async(block, buffer, left, 8 * sizeof(float)); // 左侧光晕
    cg::memcpy_async(block, buffer + 8, center, 32 * sizeof(float)); // 中心
    cg::memcpy_async(block, buffer + 40, right, 8 * sizeof(float)); // 右侧光晕
    cg::wait(block); // 等待所有复制操作完成。
    __syncthreads();
...
__global__ void stencil_kernel(const float *left, const float *center, const float *right)
{
    ...
    if (tid < 8) {
        __pipeline_memcpy_async(buffer + tid, left + tid, sizeof(float)); // 左侧光晕
    } else if (tid >= 32 - 8) {
        __pipeline_memcpy_async(buffer + tid + 16, right + tid, sizeof(float)); // 右侧光晕
    }
    if (tid < 32) {
...
`cuda::memcpy_async` 针对 `cuda::barrier` 的重载允许使用[异步屏障](../03-advanced/advanced-kernel-programming.html#advanced-kernels-advanced-sync-primitives-barriers)来同步异步数据传输。此重载通过在执行复制操作时增加当前阶段的预期计数，并在复制操作完成时减少该计数，来模拟由绑定到该屏障的另一个线程执行复制操作。这样，只有当参与屏障的所有线程都已到达，并且绑定到屏障当前阶段的所有 `memcpy_async` 操作都已完成时，`barrier` 的阶段才会推进。我们使用一个线程块范围内的 `barrier`，其中线程块内的所有线程都参与，并使用 `arrive_and_wait` 合并到达屏障和在屏障上的等待操作，因为我们在各阶段之间不执行任何工作。

请注意，我们可以使用线程级复制（版本1）或集体复制（版本2）来实现相同的结果。在版本2中，API 将自动处理底层复制操作的执行方式。在两个版本中，我们都使用 `cuda::aligned_size_t<4>()` 来通知编译器数据按4字节对齐，并且要复制的数据大小是4的倍数，以便启用 LDGSTS。请注意，为了与 `cuda::barrier` 互操作，这里使用了来自 `cuda/barrier` 头文件的 `cuda::memcpy_async`。

[cooperative_groups::memcpy_async](../05-appendices/device-callable-apis.html#cg-api-async-memcpy) 实现跨线程块内的所有线程协作协调内存传输，但使用 `cg::wait(block)` 而非显式的屏障操作来同步完成。
基于底层原语的实现使用 `__pipeline_memcpy_async()` 来启动逐元素的内存传输，使用 `__pipeline_commit()` 来提交一批复制操作，并使用 `__pipeline_wait_prior(0)` 来等待流水线中的所有操作完成。与更高级的 API 相比，这提供了最直接的控制，但代价是代码更冗长。它还确保在底层会使用 LDGSTS，而更高级的 API 则无法保证这一点。

!!! note "注意"
    在此示例中，`cooperative_groups::memcpy_async` API 的效率低于其他 API，因为它会在每次启动时立即自动提交每个复制操作，从而阻止了其他 API 所支持的、在单次提交操作之前批量处理多个复制的优化。
...
template <size_t num_stages = 2 /* 包含 num_stages 个阶段的流水线 */>
__global__ void prefetch_kernel(int* global_out, int const* global_in, size_t size, size_t batch_size) {
    auto grid = cooperative_groups::this_grid();
    auto block = cooperative_groups::this_thread_block();
    assert(size == batch_size * grid.size()); // 假设输入大小符合 batch_size * grid_size
    ...
    for (size_t compute_batch = 0, fetch_batch = num_stages; compute_batch < batch_size; ++compute_batch, ++fetch_batch) {
        ...
        __syncthreads(); // 如果每个线程处理自己复制的数据，则不需要。
        ...
        // 加载当前计算批次之前 ``num_stages`` 个阶段的未来数据。
...
- 初始化一个线程块（block）范围内的流水线。
- 通过调度 `num_stages` 个 `memcpy_async` 操作来启动流水线。
- 循环处理所有批次：它阻塞所有线程直到当前批次完成，然后对当前批次执行计算，最后如果还有下一个批次，则调度下一个 `memcpy_async`。

`cooperative_groups::memcpy_async` 的实现展示了使用 `cooperative_groups::memcpy_async` 进行多阶段数据预取。与上一个实现的主要区别在于我们不使用流水线对象，而是依赖 `cooperative_groups::memcpy_async` 在底层分阶段调度内存传输。

CUDA C 原语的实现展示了使用低级原语进行多阶段数据预取，其方式与第一个示例非常相似。

在此示例中，实现高效代码生成的一个重要细节是，即使没有更多批次需要获取，也要在流水线中保持 `num_stages` 个批次。这是通过即使没有更多批次也要提交到流水线（`pipeline.producer_commit()` 或 `__pipeline_commit()`）来实现的。请注意，由于我们无法访问内部流水线，这在协作组 API 中是不可能的。

### 4.11.1.3. 通过线程束（Warp）专用化实现生产者-消费者模式
...
{
  __mbarrier_arrive(&ready[0]); /* buffer_0 准备好进行初始填充 */
  __mbarrier_arrive(&ready[1]); /* buffer_1 准备好进行初始填充 */
  for (int i = 0; i < N / buffer_len; ++i)
  {
    __mbarrier_token_t token = __mbarrier_arrive(&filled[i % 2]);
    while(!__mbarrier_try_wait(&filled[i % 2], token, 1000)) {}
    /* 消费 buffer_(i%2) */
    __mbarrier_arrive(&ready[i % 2]); /* buffer_(i%2) 准备好被重新填充 */
  }
}
...
__global__ void producer_consumer_pattern(int N, float *in, float *out, int buffer_len)
{
  ...
  /* bar[0] 和 bar[1] 跟踪缓冲区 buffer_0 和 buffer_1 是否准备好被填充，
     而 bar[2] 和 bar[3] 分别跟踪缓冲区 buffer_0 和 buffer_1 是否已被填充 */
...
`cuda::memcpy_async` 的实现展示了最高抽象级别的 API，使用了 `cuda::memcpy_async` 和一个包含 2 个阶段的 `cuda::pipeline`。它使用了一个分区流水线（参见[流水线](../03-advanced/advanced-kernel-programming.html#advanced-kernels-advanced-sync-primitives-pipelines)），其中第一个线程束作为生产者，其余线程束作为消费者。生产者最初填充两个流水线阶段。然后，在主处理循环中，当消费者处理当前批次时，生产者获取未来批次的数据，从而维持稳定的工作流。

基于原语的 CUDA C 原语实现将 `__pipeline_memcpy_async()` 与[共享内存屏障](../03-advanced/advanced-kernel-programming.html#advanced-kernels-advanced-sync-primitives-barriers)作为完成机制相结合，以协调异步内存传输。`__pipeline_arrive_on()` 函数将内存复制与屏障关联起来。它将屏障到达计数增加一，并且当所有在其之前排序的异步操作完成后，到达计数会自动减一，因此对到达计数的净影响为零。因此，我们还需要使用 `__mbarrier_arrive()` 显式地等待屏障。
...
**命名**。张量内存加速器 (TMA) 是一个广义术语，用于指代本节描述的功能。为了保持向前兼容性并减少与 PTX ISA 的差异，本节文本根据所使用的具体复制类型，将 TMA 操作称为*批量异步复制*或*批量张量异步复制*。使用“批量”一词是为了将这些操作与上一节描述的异步内存操作区分开来。
...
```
**屏障初始化**。屏障通过参与线程块的线程数量进行初始化。因此，只有当所有线程都到达此屏障时，屏障才会翻转。共享内存屏障的更多细节在[共享内存屏障](../03-advanced/advanced-kernel-programming.html#advanced-kernels-advanced-sync-primitives-barriers)中有详细描述。

**TMA 读取**。批量异步复制指令指示硬件将一大块数据复制到共享内存中，并在完成读取后更新共享内存屏障的[事务计数](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#parallel-synchronization-and-communication-instructions-mbarrier-tracking-async-operations)。通常，发出尽可能少、尺寸尽可能大的批量复制操作能获得最佳性能。由于复制可以由硬件异步执行，因此无需将复制拆分成更小的块。

发起批量异步复制操作的线程还会告知屏障预期有多少个事务（tx）将到达。在本例中，事务以字节为单位计数。`cuda::memcpy_async` 会自动执行此操作，但 `cuda::device::memcpy_async_tx` 和 `cuda::ptx::cp_async_bulk` 不会，在使用它们之后，我们需要显式调用 `cuda::ptx::mbarrier_expect_tx`。如果多个线程更新事务计数，预期事务数将是所有更新的总和。屏障只有在所有线程都已到达 **并且** 所有字节都已到达后才会翻转。一旦屏障翻转，从共享内存读取字节就是安全的，无论是线程还是后续的批量异步复制操作都可以安全读取。关于屏障事务计数的更多信息，请参阅[跟踪异步内存操作](async-barriers.html#asynchronous-barriers-tracking)。

**屏障等待**。使用令牌通过 `bar.wait()` 来等待屏障翻转。使用屏障的显式阶段跟踪可能更高效（参见[显式阶段跟踪](async-barriers.html#asynchronous-barriers-explicit-phase)）。

**共享内存写入与同步**。缓冲区值的递增涉及对共享内存的读取和写入。为了使后续的批量异步复制操作能看到这些写入，需要使用 `cuda::ptx::fence_proxy_async` 函数。这确保了在后续通过异步代理进行读取的批量异步复制操作之前，对共享内存的写入操作被排序。因此，每个线程首先通过 `cuda::ptx::fence_proxy_async` 将对共享内存中对象的写入操作排序到异步代理中，然后所有线程的这些操作在使用 `__syncthreads()` 的线程 0 执行的异步操作之前被排序。
...
虽然使用 `if (threadIdx.x == 0)` 看起来足够，但编译器无法验证确实只有一个线程在发起复制操作，可能会插入一个遍历所有活动线程的剥离循环，这会导致线程束序列化并降低性能。为了防止这种情况，我们定义了 `is_elected()` 辅助函数，它使用 `cuda::ptx::elect_sync` 从线程束 0 中选出一个线程——这是编译器已知的——来执行复制操作，从而允许编译器生成更高效的代码。
...
__device__ inline bool is_elected()
{
    ...
    return (uniform_warp_id == 0 && ptx::elect_sync(0xFFFFFFFF)); // 在线程束 0 中选出一个领导线程。
}
...
__global__ void prefetch_kernel(int* global_out, int const* global_in, size_t size, size_t batch_size) {
    ...
    // 使用参与屏障的线程数初始化共享内存屏障。
    // 我们将为屏障使用显式的阶段跟踪，这允许我们只让一个线程到达屏障来设置事务计数，
    // 而其他线程等待基于奇偶性的阶段翻转。
    #pragma nv_diag_suppress static_var_with_dynamic_init
    ...
    for (size_t compute_batch = 0, fetch_batch = num_stages; compute_batch < batch_size; ++compute_batch, ++fetch_batch) {
        ...
        // (c) 加载当前计算批次前方 ``num_stages`` 个阶段的下一个阶段。
...
1.  **初始化阶段**：设置共享内存屏障（每个阶段一个），并将前 `num_stages` 个批次预加载到不同的共享内存区域。
2.  **主处理循环**：
    *   **等待**：使用 `mbarrier_try_wait_parity()` 等待当前批次完成拷贝。
    *   **计算**：处理当前批次的数据。
    *   **预取**：为未来的数据（保持提前 `num_stages` 个批次）调度下一个 `memcpy_async_tx` 操作。
    *   **阶段管理**：使用循环缓冲区方法在阶段间循环，并跟踪屏障奇偶性。

### 4.11.2.2. 使用 TMA 传输多维数组

在本节中，我们将重点讨论多维 TMA 拷贝。一维和多维情况的主要区别在于，必须在主机上创建张量映射并将其传递给 CUDA 内核。
...
```
**创建**。创建张量映射需要多个参数。其中包括全局内存中数组的基指针、数组大小（以元素数量计）、从一行到下一行的步幅（以字节计）、共享内存缓冲区的大小（以元素数量计）。以下代码创建了一个张量映射，用于描述大小为 `GMEM_HEIGHT x GMEM_WIDTH` 的二维行主序数组。请注意参数的顺序：变化最快的维度在前。
...
**主机到设备传输**。有三种方法可以使设备代码能够访问张量映射。推荐的方法是将张量映射作为 `const __grid_constant__` 参数传递给内核。其他可能性包括使用 `cudaMemcpyToSymbol` 将张量映射复制到设备的 `__constant__` 内存中，或通过全局内存访问它。当将张量映射作为参数传递时，某些版本的 GCC C++ 编译器会发出警告“GCC 4.6 中传递具有 64 字节对齐参数的 ABI 已更改”。可以忽略此警告。
...
```
**负索引和越界情况**。当从全局内存读取到共享内存的图块部分越界时，对应越界区域的共享内存会被零填充。图块的左上角索引也可能为负。当从共享内存写入全局内存时，图块的部分区域可能越界，但左上角不能有任何负索引。

**大小和步长**。张量的大小是指沿某一维度的元素数量。所有大小必须大于一。步长是指同一维度元素之间的字节数。例如，一个 4 x 4 的整数矩阵，其大小为 4 和 4。由于每个元素占 4 字节，其步长分别为 4 和 16 字节。由于对齐要求，一个 4 x 3 的行主序整数矩阵也必须具有 4 和 16 字节的步长。每一行会填充额外的 4 个字节，以确保下一行的起始地址对齐到 16 字节。更多关于对齐要求的信息见下表。
...
前面的章节描述了如何使用 CUDA 驱动 API 在主机上创建张量映射。

本节解释如何在设备上编码一个分片类型的张量映射。这在典型传输张量映射的方式（使用 `const __grid_constant__` 内核参数）不可取的情况下很有用，例如，在单个内核启动中处理一批不同大小的张量时。

推荐模式如下：

1.  在主机上使用驱动 API 创建一个张量映射"模板" `template_tensor_map`。
2.  在设备内核中，复制 `template_tensor_map`，修改副本，存储在全局内存中，并进行适当的栅栏操作。
3.  在内核中使用该张量映射，并进行适当的栅栏操作。
...
__global__ void encode_tensor_map(const __grid_constant__ CUtensorMap template_tensor_map, tensormap_params p, CUtensorMap* out) {
   ...
   if (threadIdx.x == 0) {
      ...
      // 对于字段 .rank，操作数 new_val 必须比期望的张量秩小一，因为该字段使用从零开始的编号。
   ...
   // 与线程束中的其他线程同步修改
   __syncwarp();
   // 与线程束中的线程一起将张量映射复制到全局内存。
   // 此外：使更新后的张量映射对设备上其他要与 cp.async.bulk 一起使用的线程可见。
...
为了确保数据在共享内存中的布局方式能够使用户代码避免共享内存存储体冲突，可以指示 TMA 引擎在将数据存储到共享内存之前对其进行“交错”，并在将数据从共享内存复制回全局内存时对其进行“反交错”。张量映射编码了“交错模式”，指示使用哪种交错模式。

示例：矩阵转置

一个例子是矩阵转置，其中数据从按行优先访问映射到按列优先访问。数据在全局内存中按行主序存储，但我们希望在共享内存中也按列访问它，这会导致存储体冲突。然而，通过使用 128 字节“交错”模式和新的共享内存索引，这些冲突可以被消除。

在此示例中，我们加载一个 `int4` 类型的 8x8 矩阵，该矩阵在全局内存中按行主序存储到共享内存。然后，每组八个线程从共享内存缓冲区加载一行，并将其存储到一个单独的转置共享内存缓冲区中的一列。这导致存储时产生八路存储体冲突。最后，将转置缓冲区写回全局内存。
为避免存储体冲突，可使用 `CU_TENSOR_MAP_SWIZZLE_128B` 布局。该布局匹配 128 字节的行长度，并以一种方式改变共享内存布局，使得列方向和行方向的访问在每次事务中都不需要相同的存储体。

下面的两个表格，[图 48](#figure-swizzle-example1) 和 [图 49](#figure-swizzle-example2)，展示了 `int4` 类型的 8x8 矩阵及其转置矩阵的正常和经过置换的共享内存布局。颜色表示矩阵元素映射到八个四存储体组中的哪一个，边距行和边距列列出了全局内存的行索引和列索引。条目显示了 16 字节矩阵元素的共享内存索引。

![未使用置换的共享内存数据布局](../images/swizzle-example1.png)

*图 48：在未使用置换的共享内存数据布局中，共享内存索引等同于全局内存索引。
每次加载指令读取一行并存储在转置缓冲区的一列中。由于转置中该列的所有矩阵元素都落在同一个存储体中，存储操作必须串行化，导致八次存储事务，从而每次存储列时产生八路存储体冲突。#*

![使用 CU_TENSOR_MAP_SWIZZLE_128B 置换的共享内存数据布局](../images/swizzle-example2.png)

*图 49：使用 CU_TENSOR_MAP_SWIZZLE_128B 置换的共享内存数据布局。一行存储在一列中，每个矩阵元素对于行和列都来自不同的存储体，因此没有任何存储体冲突。#*
...
如前所述，有四种 swizzle 模式。下表展示了不同的 swizzle 模式，包括新的共享内存索引之间的关系。这些表定义了沿 128 字节的 16 字节块到八个由四个存储体组成的子组的映射。
...
在[图 50](#figure-swizzle-overview) 中，此偏移量代表初始行偏移量，因此，在计算交错索引时，它被加到行索引 `y` 上。以下代码片段展示了如何在 `CU_TENSOR_MAP_SWIZZLE_128B` 模式下访问交错共享内存。
...
| 模式 | 交错宽度 | 共享内存框的内部维度 | 重复周期 | 共享内存对齐要求 | 全局内存对齐要求 |
| --- | --- | --- | --- | --- | --- |
...
为实现此模式，每个线程块需要2个共享内存屏障：一个用于通知消费者线程块数据已复制到共享内存缓冲区（`filled`），另一个用于通知生产者线程块消费者上的缓冲区已准备好接收数据（`ready`）。
...
- 执行集群范围的同步，以确保所有屏障在任何线程开始通信之前都已初始化。
- 每个线程确定其邻居的等级，并使用这些等级来映射远程共享内存屏障和远程共享内存缓冲区，以便向其写入数据。
