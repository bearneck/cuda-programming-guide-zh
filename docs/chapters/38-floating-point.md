...
![IEEE-754 浮点编码](../images/floating-point-ieee.drawio.png)
...
例如，值 \(-192 = (-1)^1 \times 2^7 \times 1.5\)，被编码为负号、指数 \(7\) 和小数部分 \(0.5\)。因此，对于 `float`，指数 \(7\) 由值为 `7 + 127 = 134 = 10000110` 的位串表示；对于 `double`，由 `7 + 1023 = 1030 = 10000000110` 表示。
对于 
`double`
 类型。尾数 
`0.5 = 2^-1`
...
![最小规格化值与非规格化值表示](../images/floating-point-subnormal.drawio.png)
...
| lgamma(x) | \(\ln |\Gamma(x)|\) | lgammaf(x) âª 当 \(x \notin [-10.001, -2.264]\) 时为 6 ULP âª 其他情况误差更大 | lgamma(x) âª 当 \(x \notin [-23.0001, -2.2637]\) 时为 4 ULP âª 其他情况误差更大 |
