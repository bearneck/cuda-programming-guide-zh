...
执行空间说明符 `__host__`、`__device__` 和 `__global__` 指示函数是在主机上执行还是在设备上执行。
...
内存空间说明符 `__device__`、`__managed__`、`__constant__` 和 `__shared__` 指示变量在设备上的存储位置。
...
-   `__managed__` 变量具有与动态分配的托管内存所规定的相同的连贯性和一致性行为。
-   另请参阅局部变量的限制。
...
受限指针是程序员做出的一个承诺：在该指针的生命周期内，它所指向的内存将仅通过该指针访问。这使得编译器能够执行更激进的优化。
...
因为指针 `a`、`b` 和 `c` 可能存在别名，任何通过 `c` 的写入都可能修改 `a` 或 `b` 的元素。为了保证功能正确性，编译器不能将 `a[0]` 和 `b[0]` 加载到寄存器中，将它们相乘，然后将结果同时存储在 `c[0]` 和 `c[1]` 中。这是因为如果 `a[0]` 和 `c[0]` 位于同一位置，结果将与抽象执行模型不同。编译器无法利用公共子表达式。同样，编译器不能将 `c[4]` 的计算与 `c[0]` 和 `c[1]` 的计算重新排序，因为对 `c[3]` 的先前写入可能会改变 `c[4]` 计算的输入。
...
```
请参阅 [Compiler Explorer](https://godbolt.org/z/6KeTqarnW) 上的示例。

其结果是减少了内存访问和计算量，但代价是寄存器压力因缓存加载和寄存器中的公共子表达式而增加。

由于寄存器压力是许多 CUDA 代码中的关键问题，使用受限指针可能会通过降低占用率而对性能产生负面影响。

---

对标记了 `__restrict__` 的 `__global__` 函数 `const` 指针的访问，会被编译为只读缓存加载，类似于 [PTX](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-ld-global-nc) `ld.global.nc` 指令或 `__ldg()` [低级加载/存储函数](#low-level-load-store-functions)。
...
请参阅 [Compiler Explorer](https://godbolt.org/z/drsTEPa8s) 上的示例。

### 5.4.1.5. __grid_constant__ 参数

使用 `__grid_constant__` 标注 `__global__` 函数参数，可以阻止编译器创建该参数的每个线程副本。相反，线程网格中的所有线程都将通过单个地址访问该参数，这可以提高性能。

`__grid_constant__` 参数具有以下属性：

-   其生命周期与内核相同。
-   它对单个内核是私有的，意味着其他网格（包括子网格）的线程无法访问该对象。
-   内核中的所有线程看到的地址相同。
...
请参阅 [Compiler Explorer](https://godbolt.org/z/Goq9jrEeo) 上的示例。

### 5.4.1.6. 标注摘要

下表总结了 CUDA 标注，并报告了每个标注适用的执行空间及其有效范围。

| 标注 | __host__ / __device__ / __host__ __device__ | __global__ |
| --- | --- | --- |
| __noinline__ , __forceinline__ , __inline_hint__ | 函数 | ❌ |
...
用于指定和检索沿 x、y 和 z 维度的网格和线程块的内核配置的值为 `dim3` 类型。用于获取线程块和线程索引的变量为 `uint3` 类型。`dim3` 和 `uint3` 都是简单的结构体，由三个名为 `x`、`y` 和 `z` 的无符号值组成。在 C++11 及更高版本中，`dim3` 所有分量的默认值为 1。

内置的仅设备变量：

- dim3 gridDim ：包含网格的维度，即沿 x、y 和 z 维度的线程块数量。
- dim3 blockDim ：包含线程块的维度，即沿 x、y 和 z 维度的线程数量。
- uint3 blockIdx ：包含网格内沿 x、y 和 z 维度的线程块索引。
- uint3 threadIdx ：包含线程块内沿 x、y 和 z 维度的线程索引。
- int warpSize ：一个运行时值，定义为线程束中的线程数，通常为 32。有关线程束的定义，另请参阅 Warps and SIMT。
...
*****`long` 在 C++ LLP64 数据模型（Windows 64 位）上是 4 字节，而在 C++ LP64 数据模型（Linux 64 位）上是 8 字节。
...
如果 `grid_dim` 或 `block_dim` 超过了设备允许的最大尺寸（如 [计算能力](compute-capabilities.html#compute-capabilities) 中所规定），或者 `dynamic_smem_bytes` 在考虑了静态分配的内存后大于可用的共享内存，则函数调用将失败。

### 5.4.3.1. 线程块集群

计算能力 9.0 及更高版本允许用户指定编译时线程块集群维度，以便内核可以使用 CUDA 中的 [集群层次结构](../02-basics/intro-to-cuda-cpp.html#thread-block-clusters)。编译时集群维度可以使用 `__cluster_dims__` 属性指定，语法如下：`__cluster_dims__([x, [y, [z]]])`。下面的示例展示了在 X 维度上编译时集群大小为 2，在 Y 和 Z 维度上为 1。

```cpp
__global__ void __cluster_dims__(2, 1, 1) kernel(float* parameter);
```

`__cluster_dims__()` 的默认形式指定内核将作为网格集群启动。如果未指定集群维度，用户可以在启动时指定。在启动时未指定维度将导致启动时错误。

线程块集群的维度也可以在运行时指定，并且可以使用 `cudaLaunchKernelEx` API 启动带有集群的内核。此 API 接受一个 `cudaLaunchConfig_t` 类型的配置参数、一个内核函数指针以及内核参数。以下示例展示了运行时内核配置。
...
int main() {
    ...
    attribute[0].val.clusterDim.x = 2; // X 维度的集群大小
...
```
请参阅 [Compiler Explorer](https://cuda.godbolt.org/z/M67r3a5zM) 上的示例。

### 5.4.3.2. 启动边界

正如在[内核启动与占用率](../02-basics/writing-cuda-kernels.html#writing-cuda-kernels-kernel-launch-and-occupancy)章节中所讨论的，使用更少的寄存器可以让更多的线程和线程块驻留在流式多处理器（SM）上，从而提高性能。

因此，编译器会使用启发式方法来最小化寄存器使用量，同时将[寄存器溢出](../02-basics/writing-cuda-kernels.html#writing-cuda-kernels-registers)和指令数量保持在最低水平。应用程序可以选择性地通过向编译器提供额外信息来辅助这些启发式方法，这些信息以启动边界的形式提供，通过在 `__global__` 函数的定义中使用 `__launch_bounds__()` 限定符来指定：
...
- **minBlocksPerMultiprocessor** 是可选的，指定每个流式多处理器（SM）期望的最小驻留块数；它编译为 `.minnctapersm` PTX 指令。
- **maxBlocksPerCluster** 是可选的，指定应用程序将启动 `MyKernel()` 时每个集群期望的最大线程块数；它编译为 `.maxclusterrank` PTX 指令。

如果指定了启动边界，编译器首先会推导出内核应使用的寄存器数量上限 `L`。这确保了 `maxThreadsPerBlock` 个线程的 `minBlocksPerMultiprocessor` 个块（如果未指定 `minBlocksPerMultiprocessor`，则为单个块）可以驻留在流式多处理器（SM）上。关于内核使用的寄存器数量与每个块分配的寄存器数量之间的关系，请参阅[占用率](../02-basics/writing-cuda-kernels.html#writing-cuda-kernels-kernel-launch-and-occupancy)章节。然后，编译器按如下方式优化寄存器使用：
...
如果内核在以下情况下执行，其启动将失败：
- 每个线程块的线程数超过其启动边界 `maxThreadsPerBlock`。
- 每个集群的线程块数超过其启动边界 `maxBlocksPerCluster`。

CUDA 内核所需的每个线程资源可能会以不希望的方式限制最大线程块大小。为了保持与未来硬件和工具包的向前兼容性，并确保至少有一个线程块可以在流式多处理器（SM）上运行，开发者应包含单参数 `__launch_bounds__(maxThreadsPerBlock)`，它指定了内核将启动的最大线程块大小。若不这样做，可能会导致“请求启动的资源过多”错误。在某些情况下，提供双参数版本的 `__launch_bounds__(maxThreadsPerBlock,minBlocksPerMultiprocessor)` 可以提高性能。`minBlocksPerMultiprocessor` 的最佳值应通过对每个内核的详细分析来确定。

内核的最佳启动边界通常因主要架构版本而异。以下代码示例说明了如何在设备代码中使用 `__CUDA_ARCH__`[宏](#cuda-arch-macro)来管理这一点。
...
- 要么在编译时使用不依赖于 `__CUDA_ARCH__` 的宏或常量，例如 // Host code MyKernel <<< blocksPerGrid , THREADS_PER_BLOCK >>> (...);
- 要么在运行时根据计算能力确定 // Host code cudaGetDeviceProperties ( & deviceProp , device ); int threadsPerBlock = ( deviceProp . major >= 9 ) ? 2 * THREADS_PER_BLOCK : THREADS_PER_BLOCK ; MyKernel <<< blocksPerGrid , threadsPerBlock >>> (...);
...
为了支持低级性能调优，CUDA C++ 提供了 `__maxnreg__()` 函数限定符，它将性能调优信息传递给后端优化编译器。`__maxnreg__()` 限定符指定了在线程块中可分配给单个线程的最大寄存器数。在 `__global__` 函数的定义中：
...
此函数与 `__syncthreads()` 相同，区别在于它会为线程块中所有未退出的线程计算一个谓词，并返回该谓词计算结果为非零值的线程数量。

```cpp
int __syncthreads_and(int predicate);
```

此函数与 `__syncthreads()` 相同，区别在于它会为线程块中所有未退出的线程计算一个谓词。当且仅当该谓词对所有线程的计算结果均为非零值时，它才返回一个非零值。

```cpp
int __syncthreads_or(int predicate);
```

此函数与 `__syncthreads()` 相同，区别在于它会为线程块中所有未退出的线程计算一个谓词。当且仅当该谓词对一个或多个线程的计算结果为非零值时，它才返回一个非零值。
...
内部函数 `__syncwarp()` 用于协调同一线程束内线程之间的通信。当线程束内的某些线程访问共享内存或全局内存中的相同地址时，可能会出现读后写、写后读或写后写的数据冒险。通过在两次访问之间同步线程，可以避免这些数据冒险。
...
以下示例演示了如何使用 `__syncwarp()` 来同步线程束内的线程，以安全地访问共享内存数组：
...
CUDA 编程模型采用弱序内存模型。换句话说，一个 CUDA 线程将数据写入共享内存、全局内存、页锁定主机内存或对等设备内存的顺序，不一定是另一个 CUDA 线程或主机线程观察到数据被写入的顺序。在没有内存栅栏或同步的情况下，对同一内存位置进行读取或写入会导致未定义的行为。
...
这两个线程同时对相同的内存位置 `X` 和 `Y` 进行读写。任何数据竞争都会导致未定义的行为，并且没有明确的语义。因此，`A` 和 `B` 的最终值可以是任何值。

内存栅栏和同步函数强制执行内存访问的[顺序一致性](https://en.cppreference.com/w/cpp/atomic/memory_order)。这些函数在强制执行排序的[线程作用域](https://nvidia.github.io/cccl/libcudacxx/extended_api/memory_model.html#thread-scopes)上有所不同，但与被访问的内存空间无关，包括共享内存、全局内存、页锁定主机内存和对等设备的内存。

出于安全性和可移植性考虑，建议尽可能使用 [libcu++](https://nvidia.github.io/cccl/libcudacxx/extended_api/synchronization_primitives/atomic/atomic_thread_fence.html) 提供的 `cuda::atomic_thread_fence`。
...
- 调用线程在调用 `cuda::atomic_thread_fence()` 之后对所有内存的写入，不会被设备中的任何线程观察到发生在调用线程在调用 `cuda::atomic_thread_fence()` 之前对所有内存的任何写入之前。
...
- 调用线程在调用 `__threadfence()` 之后对所有内存的写入，不会被设备中的任何线程观察到发生在调用线程在调用 `__threadfence()` 之前对所有内存的任何写入之前。
...
- 调用线程在调用 `cuda::atomic_thread_fence()` 之前对所有内存的所有写入，会被设备中的所有线程、主机线程以及对等设备中的所有线程观察到发生在调用线程在调用 `cuda::atomic_thread_fence()` 之后对所有内存的所有写入之前。
...
- 调用线程在调用 `__threadfence_system()` 之前对所有内存的所有写入，会被设备中的所有线程、主机线程以及对等设备中的所有线程观察到发生在调用线程在调用 `__threadfence_system()` 之后对所有内存的所有写入之前。
...
如果线程 1 和线程 2 属于同一个线程块，使用块级栅栏就足够了。如果线程 1 和线程 2 不属于同一个线程块，则当它们是来自同一设备的 CUDA 线程时必须使用设备级栅栏，当它们是来自两个不同设备的 CUDA 线程时必须使用系统级栅栏。

以下代码示例说明了一个常见用例，其中线程消费由其他线程产生的数据。该内核在单次调用中计算一个包含 N 个数字的数组的总和。

- 每个线程块首先对数组的一个子集求和，并将结果存储在全局内存中。
- 当所有线程块都完成后，最后一个线程块从全局内存中读取这些部分和并将它们相加，以获得最终结果。
- 为了确定哪个线程块最后完成，每个线程块原子地递增一个计数器，以表示其部分和的计算和存储已完成（更多细节请参见原子函数部分）。最后一个线程块接收到的计数器值等于 `gridDim.x - 1`。

如果在存储部分和与递增计数器之间没有内存栅栏，计数器可能在部分和存储之前就递增。这可能导致计数器达到 `gridDim.x - 1`，并允许最后一个线程块在部分和更新到内存之前就开始读取它们。

内存栅栏只影响内存操作的执行顺序；它并不保证这些操作对其他线程的可见性。

在下面的代码示例中，对 `result` 变量的内存操作的可见性是通过将其声明为 `volatile` 来确保的。更多细节，请参见 `volatile`-[限定变量](cpp-language-support.html#volatile-qualifier) 部分。
...
原子函数对共享数据执行读-修改-写操作，使其看起来像是在单一步骤中执行的。原子性确保每个操作要么完全完成，要么根本不执行，为所有参与的线程提供数据的一致视图。
CUDA 提供了四种原子函数：
...
- 它们支持 `cuda::std::atomic` 和 `cuda::std::atomic_ref` 所允许的数据类型的一个子集，128 位数据类型除外。

旧版原子函数，`atomic<Op>()`。

- 仅允许在设备代码中使用。
- 它们仅支持 `memory_order_relaxed` C++ 原子内存语义。
- 它们允许将原子操作的线程作用域作为函数名的一部分来指定。
- 与内置原子函数不同，旧版原子函数仅确保原子性，不引入同步点（内存屏障）。
- 它们支持内置原子函数所允许的数据类型的一个子集。原子加法操作支持额外的数据类型。

出于效率、安全性和可移植性考虑，建议使用 `libcu++` 提供的[扩展 CUDA C++ 原子函数](https://nvidia.github.io/cccl/libcudacxx/extended_api/synchronization_primitives.html)。

### 5.4.5.1. 旧版原子函数

旧版原子函数对存储在全局内存或共享内存中的 32 位、64 位或 128 位字执行原子读-修改-写操作。例如，`atomicAdd()` 函数读取全局内存或共享内存中特定地址的一个字，将一个数字加到该字上，然后将结果写回同一地址。

- 原子函数只能在设备函数中使用。
- 对于向量类型，如 `__half2`、`__nv_bfloat162`、`float2` 和 `float4`，读-修改-写操作在向量的每个元素上执行。不保证整个向量在单次访问中是原子的。
...
以下示例展示了 CPU 和 GPU 原子性地更新地址 `addr` 处的整数值：
...
示例请参见 [Compiler Explorer](https://godbolt.org/z/676e5bc7a)。
...
- The memory order corresponds to C++ standard atomic operationsâ memory order .
...
参数 `weak` 被忽略，它会选取 `success_order` 和 `failure_order` 之间更强的内存顺序来执行比较并交换操作。

这些函数支持以下数据类型：
-   任何大小为 2、4、8 或 16 字节的数据类型。
-   16 字节数据类型在计算能力为 9.x 及更高的设备上受支持。
...
1.  读取位于全局内存或共享内存中地址 `address` 处的旧值。
2.  `__nv_atomic_load` 将旧值存储到 `ret` 所指向的位置。`__nv_atomic_load_n` 返回旧值。
...
此原子函数根据指定的内存顺序，建立由本线程请求的内存访问之间的顺序。线程作用域参数指定了可能观察到该操作排序效果的一组线程。

## 5.4.6. 线程束函数

以下部分描述了允许线程束内的线程相互通信和执行计算的线程束函数。

出于效率、安全性和可移植性的考虑，建议尽可能使用 `CUB` 的 [Warp-Wide "Collective" Primitives](https://nvidia.github.io/cccl/cub/api_docs/warp_wide.html#warp-wide-collective-primitives) 来执行线程束操作。
...
该函数返回一个 32 位整数掩码，表示调用线程束中所有当前活动的线程。如果在调用 `__activemask()` 时线程束中的第 N 个通道处于活动状态，则第 N 位被设置。[非活动线程](../03-advanced/advanced-kernel-programming.html#simt-architecture-notes) 在返回的掩码中由 0 位表示。已退出程序的线程始终标记为非活动。

`__activemask()` 不能用于确定哪些线程束通道执行给定的分支。此函数旨在用于机会性的线程束级编程，并且仅提供线程束内活动线程的瞬时快照。
...
出于效率、安全性和可移植性的考虑，建议尽可能使用 `CUB` [Warp-Wide "Collective" Primitives](https://nvidia.github.io/cccl/cub/api/classcub_1_1WarpReduce.html#_CPPv4I0_iEN3cub10WarpReduceE) 来执行 Warp 规约。
...
建议使用 [libcu++](https://nvidia.github.io/cccl/libcudacxx/extended_api/warp/warp_shuffle.html#libcudacxx-extended-api-warp-warp-shuffle) `cuda::device::warp_shuffle()` 函数作为 `__shfl_sync()` 和 `__shfl_<op>_sync()` 内部函数的通用且更安全的替代方案。
...
Warp 洗牌函数在 [warp](../01-introduction/programming-model.html#programming-model-warps-simt) 内的非退出线程之间交换值，而无需使用共享内存。
`__shfl_sync()`
: 从指定通道直接复制。

该内置函数返回由 `srcLane` 指定 ID 的线程所持有的 `value` 值。
...
该内置函数通过从调用者的通道 ID 中减去 `delta` 来计算源通道 ID。返回由计算出的通道 ID 所持有的 `value` 值：实际上，`value` 在 warp 中向上移动了 `delta` 个通道。
...
该内置函数通过将 `delta` 加到调用者的通道 ID 上来计算源通道 ID。返回由计算出的通道 ID 所持有的 `value` 值：这具有将 `value` 在 warp 中向下移动 `delta` 个通道的效果。

- 如果 width 小于 warpSize，则线程束的每个子部分将作为一个独立的实体运行，其起始逻辑通道 ID 为 0。
- 与 __shfl_up_sync() 类似，源通道的 ID 号不会环绕 width 的值，因此较高的 delta 个通道将有效地保持不变。
...
该内置函数通过执行调用者通道 ID 与 `laneMask` 的按位异或来计算源通道 ID：返回由计算出的通道 ID 所持有的 `value` 值。此模式实现了一种蝶形寻址模式，用于树形归约和广播。

- 如果 width 小于 warpSize，则每组连续的 width 个线程能够访问来自更早组的元素。但是，如果它们尝试访问来自更晚线程组的元素，则将返回它们自己的 value 值。
...
线程只能从另一个积极参与该内置函数操作的线程读取数据。如果目标线程[处于非活动状态](../03-advanced/advanced-kernel-programming.html#simt-architecture-notes)，则检索到的值是未定义的。
...
```
### 5.4.6.6. Warp __sync 内部函数的约束

所有 Warp `__sync` 内部函数，例如：
...
都使用 `mask` 参数来指示哪些 warp 线程参与调用。此参数确保在硬件执行内部函数之前正确收敛。

`mask` 中的每一位对应一个线程的通道 ID（lane ID）。
...
在 [Compiler Explorer](https://cuda.godbolt.org/z/Tohd38edc) 上查看示例。

### 示例 3：跨 Warp 的归约

建议使用 [cub::WarpReduce](https://nvidia.github.io/cccl/cub/api/classcub_1_1WarpReduce.html) 函数来实现高效且通用的 warp 归约功能。
...
在 [Compiler Explorer](https://cuda.godbolt.org/z/T94nfGMzG) 上查看示例。
`threadIdx.x % warpSize`
）。该内部函数会等待，直到 `mask` 中指定的所有未退出的线程束线程都到达该调用点。
为确保正确执行，必须满足以下约束条件：

- 每个调用线程必须在 mask 中设置其对应的位。
- 每个非调用线程必须在 mask 中将其对应的位设置为零。已退出的线程将被忽略。
- mask 中指定的所有未退出线程必须使用相同的 mask 值执行该内部函数。
- 线程束线程可以并发地使用不同的 mask 值调用该内部函数，前提是这些 mask 是互斥的。即使在发散的控制流中，此条件也有效。

如果出现以下情况，线程束 `__sync` 函数的行为将是无效的（例如内核挂起）或未定义的：

- 调用线程未在 mask 中指定。
- mask 中指定的某个未退出线程最终未能退出，或未能在同一程序点以相同的 mask 值调用该内部函数。
- 在条件代码中，mask 中指定的所有未退出线程的条件求值结果必须相同。

当所有线程束线程都参与调用时（即当 `mask` 设置为 `0xFFFFFFFF` 时），这些内部函数的效率最高。

有效的线程束内部函数使用示例：

```cpp
__global__ void valid_examples() {
    if (threadIdx.x < 4) {        // 线程 0, 1, 2, 3 是活跃的
...
```cpp
if (threadIdx.x < 4) {           // 线程 0, 1, 2, 3 是活跃的
    __all_sync(0b0000011, pred); // 错误，线程 2, 3 是活跃的但未在 mask 中设置
    __all_sync(0b1111111, pred); // 错误，线程 4, 5, 6 不活跃但被设置在 mask 中
}
...
宏 `__CUDA_ARCH__` 表示代码正在为其编译的 NVIDIA GPU 的[虚拟架构](https://docs.nvidia.com/cuda/cuda-compiler-driver-nvcc/#virtual-architecture-macros)。它的值可能与设备的实际计算能力不同。此宏支持编写针对特定 GPU 架构优化的代码路径，这对于实现最佳性能或使用特定于架构的功能和指令可能是必需的。该宏也可用于区分主机代码和设备代码。
...
**1.** 以下实体的类型签名不应依赖于 `__CUDA_ARCH__` 是否定义，也不应依赖于其值。
...
**2.**  如果 `__global__` 函数模板从主机实例化并启动，则无论 `__CUDA_ARCH__` 是否定义或其值如何，都必须使用相同的模板参数进行实例化。
...
#if !defined(__CUDA_ARCH__)
    kernel<<<1, 1>>>(1); // 错误：仅在 __CUDA_ARCH__ 未定义时
                            //       实例化 "kernel<int>"！
#endif
}
...
在链接时只会使用 `get_ptr()` 函数的一个版本，因此行为取决于选择了哪个版本。为避免此问题，`a.cu` 和 `b.cu` 必须为相同的计算架构编译，或者不应在共享的头文件函数中使用 `__CUDA_ARCH__`。

编译器不保证会为上述不支持的 `__CUDA_ARCH__` 用法生成诊断信息。

### 5.4.7.2. __CUDA_ARCH_SPECIFIC__ 和 __CUDA_ARCH_FAMILY_SPECIFIC__

宏 `__CUDA_ARCH_SPECIFIC__` 和 `__CUDA_ARCH_FAMILY_SPECIFIC__` 分别用于识别具有[架构特定功能](compute-capabilities.html#compute-capabilities-architecture-specific-features)和[系列特定功能](compute-capabilities.html#compute-capabilities-family-specific-features)的 GPU 设备。更多信息请参见[功能集编译器目标](compute-capabilities.html#compute-capabilities-feature-set-compiler-targets)部分。
...
建议使用 [libcu++](https://nvidia.github.io/cccl/libcudacxx/extended_api/memory/is_address_from.html) 提供的 `cuda::device::is_address_from()` 和 `cuda::device::is_object_from()` 函数，作为地址空间谓词内置函数的一种可移植且更安全的替代方案。
...
如果 `ptr` 包含指定地址空间中对象的通用地址，这些函数返回 `1`，否则返回 `0`。如果参数是 `NULL` 指针，其行为是未定义的。

- __isGlobal() : 全局内存空间。
- __isShared() : 共享内存空间。
- __isConstant() : 常量内存空间。
- __isGridConstant() : 使用 `__grid_constant__` 注解的内核参数。
- __isLocal() : 本地内存空间。
...
地址空间转换函数用于在通用地址和特定地址空间中的地址之间进行转换。当编译器无法确定指针的地址空间时，例如在跨翻译单元或与 PTX 指令交互时，这些函数非常有用。
...
```
-   三个参数，带有 ReLU，比较（第一个 + 第二个）与第三个以及零。语义：max(a + b, c, 0) , max(min(a + b, c), 0)
...
这些指令根据计算能力的不同，由硬件加速或软件模拟。有关计算能力要求，请参阅[算术指令](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#arithmetic-instructions)部分。

完整的 API 可以在 [CUDA Math API 文档](https://docs.nvidia.com/cuda/cuda-math-api/cuda_math_api/group__CUDA__MATH__INTRINSIC__SIMD.html) 中找到。

---

DPX 是实现动态规划算法（例如基因组学中的 Smith-Waterman 和 Needleman-Wunsch 算法，以及路径优化中的 Floyd-Warshall 算法）的极其有用的工具。

三个有符号 32 位整数的最大值，带有 ReLU：
...
编译器优化提示用附加信息修饰代码，以帮助编译器优化生成的代码。
...
编译器默认会展开具有已知行程计数的小循环。但是，可以使用 `#pragma unroll` 指令来控制任何给定循环的展开。此指令必须紧接在循环之前，并且仅适用于该循环。

可以选择性地跟随一个整数常量表达式。以下是整数常量表达式的情况：
...
查看 [Compiler Explorer](https://godbolt.org/z/fPMK55PxE) 上的示例。

### 5.4.9.2.__builtin_assume_aligned()

建议使用 [libcu++](https://nvidia.github.io/cccl/libcudacxx/standard_api.html) ([C++ 参考](https://en.cppreference.com/w/cpp/memory/assume_aligned.html)) 提供的 `cuda::std::assume_aligned()` 函数，作为内置函数的一个可移植且更安全的替代方案。
...
该内置函数告诉编译器 `input` 预期等于 `expected`，并返回 `input` 的值。它通常用于向编译器提供分支预测信息。其行为类似于 C++20 的 `[[likely]]` 和 `[[unlikely]]`[属性](https://en.cppreference.com/w/cpp/language/attributes/likely)。
示例：
...
此内置函数告知编译器，控制流永远不会到达调用该函数的位置。如果在运行时控制流确实到达了此点，则程序行为是未定义的。

此函数可用于避免为不可达分支生成代码，并禁用编译器对不可达代码的警告。

示例：

```cpp
// 向编译器指示 default 标签永远不会被执行到。
switch (in) {
    case 1:  return 4;
    case 2:  return 10;
...
`#pragma nv_abi` 指令使在[单独编译](../02-basics/nvcc.html#nvcc-separate-compilation)模式下编译的应用程序，能够通过保留函数使用的寄存器数量，达到与[全程序编译](../02-basics/nvcc.html#nvcc-separate-compilation)相似的性能。
...
- `#pragma nv_abi` 后面的参数是可选的，可以以任意顺序提供；但是，至少需要一个参数。
- `preserve_n` 参数限制了函数调用期间保留的寄存器数量：`preserve_n_data(EXPR)` 限制数据寄存器的数量。`preserve_n_control(EXPR)` 限制控制寄存器的数量。

`#pragma nv_abi` 指令可以紧接在设备函数声明或定义之前。
...
或者，它可以放在设备函数内部的 C++ 表达式语句中的间接函数调用之前。请注意，虽然支持对自由函数的间接调用，但不支持对函数引用或类成员函数的间接调用。
...
__device__ void test() {
    ...
    #pragma nv_abi preserve_n_control(8)
    int v2 = dev_func1();            // 错误，直接调用；该编译指示无效
...
```
当应用于设备函数的声明或定义时，该编译指示会修改对该函数任何调用的自定义 ABI 属性。当置于间接函数调用点时，它仅影响该特定调用的 ABI 属性。请注意，该编译指示仅在置于调用点时影响间接函数调用；它对直接函数调用没有影响。
...
请注意，如果函数声明及其对应定义的编译指示参数不匹配，则程序格式不正确。
...
断言旨在用于调试目的。由于它们可能影响性能，建议在生产代码中禁用它们。可以通过在包含 `assert.h` 或 `<cassert>` 之前定义 `NDEBUG` 预处理器宏，或使用编译器标志 `-DNDEBUG` 在编译时禁用断言。请注意，表达式不应有副作用；否则，禁用断言将影响代码的功能。
### 5.4.10.2. 断点函数
...
C++ 线程束矩阵操作利用 Tensor Core 来加速 `D=A*B+C` 形式的矩阵问题。这些操作在计算能力 7.0 或更高的设备上支持混合精度浮点数据。这需要一个[线程束](../01-introduction/programming-model.html#programming-model-warps-simt)中所有线程的协作。此外，这些操作仅在条件代码中允许，条件是整个[线程束](../01-introduction/programming-model.html#programming-model-warps-simt)对条件的评估结果完全相同，否则代码执行很可能会挂起。

### 5.4.11.1. 描述

所有以下函数和类型都在命名空间 `nvcuda::wmma` 中定义。亚字节操作被视为预览功能，即其数据结构和 API 可能会发生变化，并且可能与未来的版本不兼容。此额外功能在 `nvcuda::wmma::experimental` 命名空间中定义。
...
一个重载类，包含分布在 warp 内所有线程上的矩阵片段。矩阵元素映射到 `fragment` 内部存储的方式是未指定的，并且可能在未来的架构中发生变化。

只允许特定的模板参数组合。第一个模板参数指定了该片段将如何参与矩阵运算。`Use` 的可接受值为：

- `matrix_a`，当片段用作第一个被乘数 A 时，
- `matrix_b`，当片段用作第二个被乘数 B 时，或
- `accumulator`，当片段用作源或目标累加器（分别为 C 或 D）时。

m、n 和 k 的大小描述了参与乘加运算的 warp 范围矩阵图块的形状。每个图块的维度取决于其角色。对于 `matrix_a`，图块维度为 m x k；对于 `matrix_b`，维度为 k x n；累加器图块维度为 m x n。

数据类型 `T`，对于被乘数可以是 `double`、`float`、`__half`、`__nv_bfloat16`、`char` 或 `unsigned char`；对于累加器可以是 `double`、`float`、`int` 或 `__half`。如《元素类型和矩阵大小》文档所述，仅支持有限的累加器和被乘数类型组合。

对于 `matrix_a` 和 `matrix_b` 片段，必须指定 `Layout` 参数。`row_major` 或 `col_major` 分别表示矩阵行或列中的元素在内存中是连续的。累加器矩阵的 `Layout` 参数应保留默认值 `void`。仅当如下所述加载或存储累加器时，才指定行或列布局。

`load_matrix_sync`

等待直到 warp 中的所有通道都到达 `load_matrix_sync`，然后从内存加载矩阵片段 `a`。`mptr` 必须是一个 256 位对齐的指针，指向内存中矩阵的第一个元素。`ldm` 描述了连续行（对于行主序布局）或列（对于列主序布局）之间的元素跨度，对于 `__half` 元素类型必须是 8 的倍数，对于 `float` 元素类型必须是 4 的倍数（即，两种情况下都是 16 字节的倍数）。如果片段是 `accumulator`，则必须将 `layout` 参数指定为 `mem_row_major` 或 `mem_col_major`。对于 `matrix_a` 和 `matrix_b` 片段，布局是从片段的 `layout` 参数推断出来的。`mptr`、`ldm`、`layout` 以及 `a` 的所有模板参数的值对于 warp 中的所有线程必须相同。此函数必须由 warp 中的所有线程调用，否则结果未定义。

`store_matrix_sync`

等待直到 warp 中的所有通道都到达 `store_matrix_sync`，然后将矩阵片段 `a` 存储到内存。`mptr` 必须是一个 256 位对齐的指针，指向内存中矩阵的第一个元素。`ldm` 描述了连续行（对于行主序布局）或列（对于列主序布局）之间的元素跨度，对于 `__half` 元素类型必须是 8 的倍数，对于 `float` 元素类型必须是 4 的倍数（即，两种情况下都是 16 字节的倍数）。输出矩阵的布局必须指定为 `mem_row_major` 或 `mem_col_major`。`mptr`、`ldm`、`layout` 以及 `a` 的所有模板参数的值对于 warp 中的所有线程必须相同。
`fill_fragment`

用常量值 `v` 填充矩阵片段。由于矩阵元素到每个片段的映射是未指定的，此函数通常由线程束中的所有线程调用，且 `v` 的值对所有线程应相同。
...
由于矩阵元素到每个线程的 `fragment` 的映射是未指定的，因此在调用 `store_matrix_sync` 之后，必须从内存（共享内存或全局内存）中访问各个矩阵元素。在特殊情况下，如果线程束中的所有线程将对所有片段元素统一应用逐元素操作，则可以使用以下 `fragment` 类成员直接访问元素。
...
例如，以下代码将一个 `accumulator` 矩阵图块缩放一半。
...
此数据格式是一种替代的 fp16 格式，其范围与 f32 相同，但精度降低（7 位）。您可以直接使用 `cuda_bf16.h` 中提供的 `__nv_bfloat16` 类型。具有 `__nv_bfloat16` 数据类型的矩阵片段需要与 `float` 类型的累加器组合使用。支持的形状和操作与 `__half` 相同。

`tf32`

此数据格式是 Tensor Core 支持的一种特殊浮点格式，其范围与 f32 相同，但精度降低（>=10 位）。此格式的内部布局由实现定义。要将此浮点格式用于 WMMA 操作，必须手动将输入矩阵转换为 tf32 精度。

为便于转换，提供了一个新的内部函数 `__float_to_tf32`。虽然该内部函数的输入和输出参数是 `float` 类型，但输出在数值上将是 `tf32`。这种新精度仅用于 Tensor Core，如果与其他 `float` 类型操作混合使用，结果的精度和范围将是未定义的。
一旦输入矩阵（`matrix_a` 或 `matrix_b`）被转换为 tf32 精度，将 `precision::tf32` 精度的 `fragment` 与 `float` 数据类型结合用于 `load_matrix_sync` 将利用这一新功能。两个累加器片段都必须具有 `float` 数据类型。唯一支持的矩阵尺寸是 16x16x8 (m-n-k)。
...
亚字节 WMMA 操作提供了一种访问 Tensor Core 低精度能力的方式。它们被视为预览功能，即其数据结构和 API 可能会发生变化，并且可能与未来的版本不兼容。此功能可通过 `nvcuda::wmma::experimental` 命名空间使用：
...
namespace experimental {
    ...
    enum bmmaBitOp {
        bmmaBitOpXOR = 1, // compute_75 最低要求
        bmmaBitOpAND = 2  // compute_80 最低要求
    };
...
对于亚字节操作，`load_matrix_sync` 中的 `ldm` 值对于元素类型 `experimental::precision::u4` 和 `experimental::precision::s4` 应为 32 的倍数，对于元素类型 `experimental::precision::b1` 应为 128 的倍数（即，两种情况均为 16 字节的倍数）。

对以下 MMA 指令变体的支持已被弃用，并将在 sm_90 中移除：
...
等待所有线程束通道都执行了 `bmma_sync`，然后执行线程束同步的位矩阵乘积累加操作 `D = (A op B) + C`，其中 `op` 包含一个逻辑操作 `bmmaBitOp` 和由 `bmmaAccumulateOp` 定义的累加操作。可用的操作有：

`bmmaBitOpXOR`，对 `matrix_a` 中的一行与 `matrix_b` 中的 128 位列执行 128 位异或操作。

`bmmaBitOpAND`，对 `matrix_a` 中的一行与 `matrix_b` 中的 128 位列执行 128 位与操作，此操作在计算能力 8.0 及更高的设备上可用。

累加操作始终是 `bmmaAccumulateOpPOPC`，它计算置位位的数量。

### 5.4.11.5. 限制

张量核心所需的特殊格式可能因每个主要和次要设备架构而异。由于线程仅持有整个矩阵的一个片段（不透明的、特定于架构的 ABI 数据结构），开发者不允许对各个参数如何映射到参与矩阵乘积累加操作的寄存器做出假设，这使得情况更加复杂。

由于片段是特定于架构的，如果函数 A 和函数 B 已为不同的链接兼容架构编译并链接到同一个设备可执行文件中，那么将片段从函数 A 传递到函数 B 是不安全的。在这种情况下，片段的大小和布局将特定于一种架构，而在另一种架构中使用 WMMA API 将导致不正确的结果，甚至可能造成数据损坏。

片段布局不同的两个链接兼容架构的一个例子是 sm_70 和 sm_75。
...
这种未定义的行为在编译时和运行时工具中也可能无法检测到，因此需要格外小心以确保片段的布局是一致的。当链接到一个既为不同的链接兼容架构构建又期望接收 WMMA 片段的旧版库时，最有可能出现这种链接风险。

请注意，在弱链接的情况下（例如，CUDA C++ 内联函数），链接器可能会选择任何可用的函数定义，这可能导致编译单元之间的隐式传递。

为了避免这类问题，矩阵应始终存储到内存中，以便通过外部接口传输（例如 `wmma::store_matrix_sync(dst, …);`），然后可以安全地将其作为指针类型（例如 `float *dst`）传递给 `bar()`。

请注意，由于 sm_70 可以在 sm_75 上运行，上述示例中的 sm_75 代码可以更改为 sm_70，并在 sm_75 上正确工作。但是，当与其他单独编译的 sm_75 二进制文件链接时，建议在您的应用程序中使用 sm_75 原生代码。
### 5.4.11.6. 元素类型与矩阵尺寸

张量核心支持多种元素类型和矩阵尺寸。下表列出了支持的 `matrix_a`、`matrix_b` 和 `accumulator` 矩阵的各种组合：
