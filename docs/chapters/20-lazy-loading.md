...
延迟加载通过等待需要时才加载 CUDA 模块，从而减少程序初始化时间。延迟加载对于只使用其包含的少量内核的程序特别有效，这在库使用时很常见。当遵循 CUDA 编程模型时，延迟加载被设计为对用户不可见。[潜在风险](#lazy-loading-potential-hazards)对此进行了详细说明。从 CUDA 12.3 开始，延迟加载在所有平台上默认启用，但可以通过 `CUDA_MODULE_LOADING` 环境变量进行控制。
...
延迟加载从 CUDA 运行时版本 11.7 开始可用。由于 CUDA 运行时通常静态链接到程序和库中，只有来自 CUDA 11.7+ 工具包或使用其编译的程序和库才能受益于延迟加载。使用较旧 CUDA 运行时版本编译的库将急切地加载所有模块。
...
延迟加载不需要任何编译器支持。使用 11.7 之前编译器编译的 SASS 和 PTX 都可以在启用延迟加载的情况下加载，并将获得该功能的全部好处。但是，如上所述，仍然需要 11.7+ 版本的 CUDA 运行时。
...
延迟加载的设计使其无需对应用程序进行任何修改即可使用。尽管如此，仍有一些注意事项，特别是当应用程序不完全符合 CUDA 编程模型时，如下所述。

### 4.7.5.1. 对并发内核执行的影响

某些程序错误地假设并发内核执行是有保证的。如果需要跨内核同步，但内核执行已被序列化，则可能发生死锁。为了最小化延迟加载对并发内核执行的影响，请执行以下操作：
...
延迟加载将 CUDA 模块的内存分配从程序初始化推迟到更接近执行时间。如果应用程序在启动时就分配了整个 VRAM，CUDA 可能在运行时无法为模块分配内存。可能的解决方案：
...
延迟加载可能会将 CUDA 模块初始化移到测量的执行窗口内，从而影响性能测量结果。为避免这种情况：
