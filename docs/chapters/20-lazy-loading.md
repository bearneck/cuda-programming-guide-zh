...
延迟加载通过等待 CUDA 模块直到需要时才加载，从而减少程序初始化时间。延迟加载对于仅使用其包含的少量内核的程序特别有效，这在库使用时很常见。当遵循 CUDA 编程模型时，延迟加载被设计为对用户不可见。[潜在风险](#lazy-loading-potential-hazards)对此进行了详细说明。从 CUDA 12.3 开始，延迟加载在所有平台上默认启用，但可以通过 `CUDA_MODULE_LOADING` 环境变量进行控制。
...
延迟加载不需要任何编译器支持。使用 11.7 之前的编译器编译的 SASS 和 PTX 都可以在启用延迟加载的情况下加载，并将获得该功能的全部好处。但是，如上所述，仍然需要 11.7+ 版本的 CUDA 运行时。
...
延迟加载可能会将 CUDA 模块初始化过程移入测量的执行窗口，从而影响性能测量的准确性。为避免这种情况：
