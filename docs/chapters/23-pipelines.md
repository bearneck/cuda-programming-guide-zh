...
本节主要介绍如何通过 `cuda::pipeline` API 使用流水线（并在适用时提供指向原语的指引）。
...
流水线可以是*统一的*或*分区的*。在统一流水线中，所有参与的线程既是生产者也是消费者。在分区流水线中，每个参与的线程要么是生产者，要么是消费者，并且其角色在流水线对象的生命周期内不能改变。线程本地流水线不能被分区。要创建分区流水线，我们需要向 `cuda::make_pipeline()` 提供生产者数量或线程角色。
...
为了支持分区，共享的 `cuda::pipeline` 会产生额外的开销，包括每个阶段使用一组共享内存屏障进行同步。即使流水线是统一的并且可以使用 `__syncthreads()` 替代，这些开销仍然存在。因此，在可能的情况下，最好使用线程本地流水线，以避免这些开销。

## 4.10.2. 提交工作

将工作提交到流水线阶段涉及以下步骤：

> 生产者线程集合使用 `pipeline.producer_acquire()` 获取流水线头部。
...
流水线机制在同一线程束内的 CUDA 线程间共享。这种共享导致提交的操作序列在线程束内纠缠，在某些情况下可能影响性能。
...
- 该线程束每个线程的感知序列为：线程 0: TB = {0} ( TL=0 ) 线程 1: TB = {0} ( TL=0 ) … 线程 31: TB = {0} ( TL=0 )
...
*等待先前* 的变体等待**实际**序列中至少到并包括 `PL-N` 的批次。由于 `TL <= PL`，等待到并包括 `PL-N` 的批次包含了等待批次 `TL-N`。因此，当 `TL < PL` 时，线程将无意中等待额外的、更新的批次。在上面极端完全发散的线程束示例中，每个线程可能等待所有 32 个批次。

!!! note "注意"
    建议通过收敛的线程调用提交操作，以避免过度等待，保持线程感知的批次序列与实际序列对齐。当这些操作之前的代码导致线程发散时，应在调用提交操作之前通过 `__syncwarp` 重新收敛线程束。
## 4.10.5. 提前退出
